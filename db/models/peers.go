...
type peerR struct {
	...
	Keys           KeySlice          `boil:"Keys" json:"Keys" toml:"Keys" yaml:"Keys"`
...
var (
	peerAllColumns            = []string{"id", "agent_version_id", "protocols_set_id", "multi_hash", "updated_at", "created_at", "last_seen_at"}
	peerColumnsWithoutDefault = []string{"multi_hash", "updated_at", "created_at", "last_seen_at"}
...
func (o *Peer) Keys(mods ...qm.QueryMod) keyQuery {
...
func (peerL) LoadKeys(ctx context.Context, e boil.ContextExecutor, singular bool, maybePeer interface{}, mods queries.Applicator) error {
	var slice []*Peer
	var object *Peer
	...
	query := NewQuery(
		qm.From(`keys`),
		qm.WhereIn(`keys.peer_id in ?`, args...),
	)
	if mods != nil {
		mods.Apply(query)
	}
	...
	for _, foreign := range resultSlice {
		for _, local := range slice {
			if queries.Equal(local.ID, foreign.PeerID) {
...
func (o *Peer) AddKeys(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Key) error {
	var err error
	for _, rel := range related {
		if insert {
			queries.Assign(&rel.PeerID, o.ID)
...
// SetKeys removes all previously related items of the
// peer replacing them completely with the passed
// in related items, optionally inserting them as new records.
// Sets o.R.Peer's Keys accordingly.
// Replaces o.R.Keys with related.
// Sets related.R.Peer's Keys accordingly.
func (o *Peer) SetKeys(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Key) error {
	query := "update \"keys\" set \"peer_id\" = null where \"peer_id\" = $1"
	...
	if o.R != nil {
		for _, rel := range o.R.Keys {
			queries.SetScanner(&rel.PeerID, nil)
			if rel.R == nil {
				continue
			}
	...
	return o.AddKeys(ctx, exec, insert, related...)
}

// RemoveKeys relationships from objects passed in.
// Removes related items from R.Keys (uses pointer comparison, removal does not keep order)
// Sets related.R.Peer.
func (o *Peer) RemoveKeys(ctx context.Context, exec boil.ContextExecutor, related ...*Key) error {
	if len(related) == 0 {
		return nil
	}
	...
	for _, rel := range related {
		...
		if _, err = rel.Update(ctx, exec, boil.Whitelist("peer_id")); err != nil {
			return err
		}
	}
	if o.R == nil {
	...
	for _, rel := range related {
		for i, ri := range o.R.Keys {
			...
			ln := len(o.R.Keys)
			if ln > 1 && i < ln-1 {
				o.R.Keys[i] = o.R.Keys[ln-1]
			}
			o.R.Keys = o.R.Keys[:ln-1]
			break
		}
	}
